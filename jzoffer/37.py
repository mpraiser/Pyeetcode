    ...
    def serialize(self, root):
        ...
        while len(dq) > 0:
            ...
            if node.right:
                ...
                values.append(None)
        values = values[:2 ** max_depth - 1]
        return self.list2str(values)

    def deserialize(self, data):
        values = self.str2list(data)
        if not values:
            return None
...
if __name__ == "__main__":
    ...
    for case in cases:
        tree = TreeNode.from_str(case)
        # print(TreeNode.to_str(tree))

        codec = Codec()
        ser = codec.serialize(tree)
        # print(ser)
        tree = codec.deserialize(ser)
